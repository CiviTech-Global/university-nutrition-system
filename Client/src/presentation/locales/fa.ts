export const fa = {
  title: "سیستم تغذیه دانشگاهی",
  subtitle: "حساب کاربری خود را ایجاد کنید",
  firstName: "نام",
  lastName: "نام خانوادگی",
  email: "ایمیل",
  username: "نام کاربری",
  password: "رمز عبور",
  confirmPassword: "تأیید رمز عبور",
  createAccount: "ایجاد حساب کاربری",
  alreadyHaveAccount: "قبلاً حساب کاربری دارید؟",
  signInHere: "اینجا وارد شوید",
  // Validation messages
  firstNameRequired: "نام الزامی است",
  lastNameRequired: "نام خانوادگی الزامی است",
  emailRequired: "ایمیل الزامی است",
  emailInvalid: "لطفاً یک آدرس ایمیل معتبر وارد کنید",
  usernameRequired: "نام کاربری الزامی است",
  usernameMinLength: "نام کاربری باید حداقل 3 کاراکتر باشد",
  passwordRequired: "رمز عبور الزامی است",
  passwordMinLength: "رمز عبور باید حداقل 6 کاراکتر باشد",
  confirmPasswordRequired: "لطفاً رمز عبور خود را تأیید کنید",
  passwordsDoNotMatch: "رمزهای عبور مطابقت ندارند",
  // Additional validation messages
  usernameTaken: "این نام کاربری قبلاً استفاده شده است",
  emailRegistered: "این ایمیل قبلاً ثبت شده است",
  accountCreated: "حساب کاربری با موفقیت ایجاد شد!",
  creatingAccount: "در حال ایجاد...",
  errorCreatingAccount: "خطا در ایجاد حساب کاربری",
  // Login translations
  loginTitle: "خوش آمدید",
  loginSubtitle: "به حساب کاربری خود وارد شوید",
  loginButton: "ورود",
  loggingIn: "در حال ورود...",
  forgotPassword: "رمز عبور را فراموش کرده‌اید؟",
  dontHaveAccount: "حساب کاربری ندارید؟",
  registerHere: "اینجا ثبت نام کنید",
  // Login validation
  invalidCredentials: "نام کاربری یا رمز عبور اشتباه است",
  loginSuccess: "ورود موفقیت‌آمیز بود!",
  loginError: "خطا در ورود",
  // Forgot Password translations
  forgotPasswordTitle: "بازیابی رمز عبور",
  forgotPasswordSubtitle:
    "ایمیل و رمز عبور جدید خود را وارد کنید تا حساب کاربری خود را بازنشانی کنید",
  resetPasswordButton: "تغییر رمز عبور",
  sendingResetLink: "در حال ارسال...",
  backToLogin: "بازگشت به صفحه ورود",
  // Forgot Password validation
  emailSent: "ایمیل بازیابی رمز عبور با موفقیت ارسال شد!",
  emailNotFound: "ایمیل در سیستم یافت نشد",
  forgotPasswordError: "خطا در ارسال ایمیل بازیابی",
  passwordResetSuccess: "رمز عبور با موفقیت تغییر یافت!",
  newPassword: "رمز عبور جدید",
  resettingPassword: "در حال تغییر...",
  // Dashboard translations
  welcomeMessage: "خوش آمدید",
  userInformation: "اطلاعات کاربری",
  memberSince: "تاریخ عضویت",
  dashboardContent: "محتوای داشبورد",
  dashboardDescription:
    "اینجا محتوای اصلی داشبورد سیستم تغذیه دانشگاهی قرار خواهد گرفت.",
  logout: "خروج",
  dashboard: "داشبورد",
  mealsThisWeek: "وعده‌های غذایی این هفته",
  availableCredit: "اعتبار موجود",
  newNotifications: "اعلان‌های جدید",
  attendanceRate: "نرخ حضور",
  // Credit translations
  credit: "اعتبار",
  addCredit: "افزودن اعتبار",
  currentBalance: "موجودی فعلی",
  recentTransactions: "تراکنش‌های اخیر",
  creditAdded: "اعتبار اضافه شد",
  mealPurchase: "خرید وعده غذایی",
  debit: "برداشت",
  amount: "مبلغ",
  cancel: "لغو",
  add: "افزودن",
  // Profile translations
  profile: "پروفایل",
  edit: "ویرایش",
  save: "ذخیره",
  personalInformation: "اطلاعات شخصی",
  // Meal Reservation translations
  mealReservation: "رزرو وعده غذایی",
  day: "روز",
  breakfast: "صبحانه",
  lunch: "ناهار",
  dinner: "شام",
  today: "امروز",
  selectMeal: "وعده غذایی انتخاب کنید",
  // Additional translations
  totalCost: "هزینه کل",
  previousWeek: "هفته قبل",
  nextWeek: "هفته بعد",
  saveReservations: "ذخیره رزروها",
  saving: "در حال ذخیره...",
  saveButton: "ذخیره",
  reservationsSaved: "رزروها با موفقیت ذخیره شدند!",
  errorSavingReservations: "خطا در ذخیره رزروها. لطفاً دوباره تلاش کنید.",
  past: "گذشته",
  refreshData: "تازه‌سازی داده‌ها",
  creditAddedSuccess: "اعتبار با موفقیت اضافه شد!",
  errorAddingCredit: "خطا در افزودن اعتبار. لطفاً دوباره تلاش کنید.",
  totalCredits: "کل اعتبارات",
  totalDebits: "کل برداشت‌ها",
  thisMonth: "این ماه",
  loading: "در حال بارگذاری...",
  userNotFound: "کاربر یافت نشد. لطفاً دوباره وارد شوید.",
  // Currency and financial terms
  currency: "تومان",
  dollar: "دلار",
  creditTerm: "اعتبار",
  balance: "موجودی",
  amountField: "مبلغ",
  price: "قیمت",
  cost: "هزینه",
  payment: "پرداخت",
  transaction: "تراکنش",
  deposit: "واریز",
  withdrawal: "برداشت",
  // Food and meal related terms
  food: "غذا",
  foods: "غذاها",
  meal: "وعده غذایی",
  breakfastMeal: "صبحانه",
  lunchMeal: "ناهار",
  dinnerMeal: "شام",
  snack: "میان‌وعده",
  beverage: "نوشیدنی",
  dessert: "دسر",
  appetizer: "پیش غذا",
  mainCourse: "غذای اصلی",
  sideDish: "غذای جانبی",
  vegetarian: "گیاهخواری",
  vegan: "وگان",
  halal: "حلال",
  kosher: "کوشر",
  glutenFree: "بدون گلوتن",
  dairyFree: "بدون لبنیات",
  nutFree: "بدون آجیل",
  spicy: "تند",
  mild: "ملایم",
  hot: "داغ",
  cold: "سرد",
  fresh: "تازه",
  organic: "ارگانیک",
  // Additional UI translations
  week: "هفته",
  currentWeekUI: "هفته جاری",
  nextWeekUI: "هفته بعد",
  previousWeekUI: "هفته قبل",
  noReservations: "هنوز رزروی وجود ندارد",
  reservationCount: "تعداد رزروها",
  totalSpent: "کل هزینه",
  averagePerMeal: "متوسط هر وعده",
  mostPopularMeal: "محبوب‌ترین وعده",
  leastPopularMeal: "کم‌محبوب‌ترین وعده",
  mealPreferences: "ترجیحات غذایی",
  dietaryRestrictions: "محدودیت‌های غذایی",
  specialRequests: "درخواست‌های ویژه",
  confirmReservation: "تأیید رزرو",
  cancelReservation: "لغو رزرو",
  reservationConfirmed: "رزرو با موفقیت تأیید شد!",
  reservationCancelled: "رزرو با موفقیت لغو شد!",
  errorConfirmingReservation: "خطا در تأیید رزرو. لطفاً دوباره تلاش کنید.",
  errorCancellingReservation: "خطا در لغو رزرو. لطفاً دوباره تلاش کنید.",
  noTransactions: "تراکنشی یافت نشد",
  transactionHistory: "تاریخچه تراکنش‌ها",
  filterByDateTransaction: "فیلتر بر اساس تاریخ",
  filterByType: "فیلتر بر اساس نوع",
  exportData: "خروجی داده",
  importData: "ورودی داده",
  settings: "تنظیمات",
  notifications: "اعلان‌ها",
  help: "راهنما",
  about: "درباره",
  contactSupport: "تماس با پشتیبانی",
  feedback: "بازخورد",
  reportIssue: "گزارش مشکل",
  termsOfService: "شرایط استفاده",
  privacyPolicy: "سیاست حریم خصوصی",
  // Sidebar translations
  expand: "گسترش",
  collapse: "جمع‌شدن",
  mainMenu: "منوی اصلی",
  tools: "ابزارها",
  language: "زبان",
  forgotPasswordMenu: "بازیابی رمز عبور",
  saleDay: "روز فروش",
  // Sale Day translations
  saleDayTitle: "رزرو اضطراری غذا",
  saleDaySubtitle: "رزروهای آخرین لحظه برای امروز",
  availableToday: "موجود امروز",
  emergencyReservation: "رزرو اضطراری",
  quickReserve: "رزرو سریع",
  reserveNow: "رزرو کنید",
  soldOut: "تمام شده",
  limitedAvailability: "موجودی محدود",
  lastChance: "آخرین فرصت",
  todayOnly: "فقط امروز",
  noReservationsNeeded: "برای امروز نیازی به رزرو نیست",
  walkInAvailable: "ورود آزاد موجود است",
  instantConfirmation: "تأیید فوری",
  emergencyFeeNote: "هزینه اضطراری ممکن است اعمال شود",
  remainingQuantity: "باقی‌مانده",
  reserveForToday: "رزرو برای امروز",
  confirmEmergencyReservation: "تأیید رزرو اضطراری",
  emergencyReservationSuccess: "رزرو اضطراری تأیید شد!",
  emergencyReservationError: "خطا در رزرو اضطراری",
  emergencyReservationNote: "توجه: رزروهای اضطراری مشروط به موجودی است",
  // Test Language translations
  typography: "تایپوگرافی",
  formatting: "قالب‌بندی",
  components: "اجزا",
  // Announcements translations
  announcements: "اعلان‌ها",
  noAnnouncements: "در حال حاضر اعلانی وجود ندارد",
  holidayNotice: "اعلان تعطیلات",
  serviceSuspension: "تعلیق خدمات",
  maintenanceNotice: "اعلان نگهداری",
  specialOffer: "پیشنهاد ویژه",
  importantUpdate: "به‌روزرسانی مهم",
  systemMaintenance: "نگهداری سیستم",
  // Enhanced authentication translations
  welcomeBack: "خوش آمدید",
  signInToContinue: "برای ادامه به حساب کاربری خود وارد شوید",
  secureLogin: "ورود امن",
  loginWithConfidence: "با اطمینان وارد شوید",
  systemStatus: "وضعیت سیستم",
  allSystemsOperational: "تمام سیستم‌ها فعال هستند",
  maintenanceScheduled: "نگهداری برنامه‌ریزی شده",
  // Error messages
  networkError: "خطای شبکه. لطفاً اتصال خود را بررسی کنید.",
  serverError: "خطای سرور. لطفاً دوباره تلاش کنید.",
  accountLocked: "حساب کاربری به دلیل تلاش‌های مکرر ناموفق موقتاً قفل شده است.",
  accountDisabled:
    "حساب کاربری غیرفعال شده است. لطفاً با پشتیبانی تماس بگیرید.",
  sessionExpired: "جلسه شما منقضی شده است. لطفاً دوباره وارد شوید.",
  // Success messages
  loginRedirecting: "ورود موفقیت‌آمیز بود! در حال انتقال به داشبورد...",
  // Additional UI elements
  rememberMe: "مرا به خاطر بسپار",
  staySignedIn: "وارد بمان",
  loginOptions: "گزینه‌های ورود",
  quickLogin: "ورود سریع",
  biometricLogin: "ورود بیومتریک",
  twoFactorAuth: "احراز هویت دو مرحله‌ای",
  // System information
  lastLogin: "آخرین ورود",
  accountActivity: "فعالیت حساب کاربری",
  securitySettingsInfo: "تنظیمات امنیتی",
  privacyNotice: "اعلان حریم خصوصی",
  termsOfUse: "شرایط استفاده",
  supportContact: "تماس با پشتیبانی",
  // Weekly Schedule translations
  weeklySchedule: "برنامه هفتگی غذا",
  weekTotalSchedule: "هزینه کل هفته",
  selectFood: "غذا انتخاب کنید",
  selectCafeteria: "سلف انتخاب کنید",
  paymentConfirmation: "تأیید رزرو و پرداخت",
  reservationDetails: "جزئیات رزرو",
  priceCalculation: "محاسبه قیمت",
  basePrice: "قیمت پایه",
  emergencyFee: "هزینه اضطراری",
  finalAmount: "مبلغ نهایی",
  applyDiscountCode: "اعمال کد تخفیف",
  payAndConfirm: "پرداخت و تأیید",
  reservationConfirmedSchedule: "رزرو با موفقیت تأیید شد!",
  faramushiCode: "کد فرموشی",
  // My Reservations translations
  myReservations: "رزروهای من",
  allReservations: "همه",
  upcomingReservations: "آینده",
  pastReservations: "گذشته",
  reservationInfo: "اطلاعات رزرو",
  nutritionalInfo: "اطلاعات تغذیه‌ای",
  ingredients: "مواد تشکیل دهنده",
  reservationQRCode: "QR کد رزرو",
  showQRCode: "این QR کد را هنگام دریافت غذا نشان دهید",
  copyCode: "کپی کد",
  downloadReceipt: "دانلود رسید",
  receiptDownloaded: "رسید دانلود شد",
  codeCopied: "کد کپی شد",
  noReservationsFound: "هیچ رزروی یافت نشد",
  noUpcomingReservations: "هیچ رزرو آینده‌ای یافت نشد",
  noPastReservations: "هیچ رزرو گذشته‌ای یافت نشد",
  filterByStatus: "فیلتر بر اساس وضعیت",
  all: "همه",
  confirmed: "تأیید شده",
  completed: "تکمیل شده",
  cancelled: "لغو شده",
  totalCostReservation: "هزینه کل",
  popular: "محبوب",
  vegetarianReservation: "گیاهی",
  // Enhanced SaleDay translations
  confirmEmergencyReservationSale: "تأیید رزرو اضطراری",
  selectRestaurant: "رستوران انتخاب کنید",
  applyDiscount: "اعمال",
  emergencyReservationConfirmed: "رزرو اضطراری با موفقیت تأیید شد!",
  paymentProcessingError: "خطا در پردازش پرداخت",
  insufficientBalance: "موجودی کافی نیست",
  invalidDiscountCode: "کد تخفیف نامعتبر است",
  discountApplied: "% تخفیف اعمال شد",
  availableCodes: "کدهای موجود",
  onlinePaymentGateway: "درگاه پرداخت آنلاین",
  wallet: "کیف پول",
  // Account Recharge translations
  accountRecharge: "شارژ حساب",
  rechargeAccount: "شارژ حساب کاربری",
  accountRechargeTitle: "شارژ حساب کاربری",
  selectAmount: "انتخاب مبلغ (تومان)",
  customAmount: "مبلغ دلخواه",
  selectPaymentGateway: "انتخاب درگاه پرداخت",
  paymentGateway: "درگاه پرداخت",
  paymentSummary: "خلاصه پرداخت",
  rechargeAmount: "مبلغ شارژ",
  gatewayFee: "کارمزد درگاه",
  totalAmount: "مبلغ کل",
  proceedToPayment: "ادامه پرداخت",
  confirmPayment: "تأیید پرداخت",
  payNow: "پرداخت",
  processingPayment: "در حال پردازش پرداخت...",
  paymentSuccessful: "پرداخت موفق!",
  accountRechargeSuccess: "حساب شما با موفقیت شارژ شد!",
  paymentFailed: "پرداخت ناموفق",
  transactionDetails: "جزئیات تراکنش",
  rechargeAccountDescription: "برای رزرو غذا، حساب خود را شارژ کنید",
  minimumAmount: "حداقل ۱۰٬۰۰۰ و حداکثر ۲۰۰٬۰۰۰ تومان",
  amountRange: "مبلغ باید بین ۱۰٬۰۰۰ تا ۲۰۰٬۰۰۰ تومان باشد",
  paymentProcessingNote: "لطفاً منتظر بمانید. این فرآیند ممکن است چند ثانیه طول بکشد.",
  securePaymentNote: "شما در حال انتقال به درگاه پرداخت امن هستید.",
  canProceedReservations: "اکنون می‌توانید برای رزرو غذا اقدام کنید.",
  goBack: "بازگشت",
  refreshBalance: "بروزرسانی موجودی",
  currentAccountBalance: "موجودی فعلی حساب شما",
  fee: "کارمزد",
  total: "مجموع",
  continue: "ادامه",
  // Payment Gateway specific
  mellatBank: "بانک ملت",
  parsianBank: "بانک پارسیان",
  zarinPal: "زرین‌پال",
  samanBank: "بانک سامان",
  melliBank: "بانک ملی",
  // Settings page translations
  generalSettings: "تنظیمات عمومی",
  accountSettings: "تنظیمات حساب کاربری",
  notificationSettingsMenu: "تنظیمات اعلان‌ها",
  securitySettings: "تنظیمات امنیتی",
  displaySettings: "تنظیمات نمایش",
  dataSettings: "داده‌ها و حریم خصوصی",
  languageSettings: "تنظیمات زبان",
  changeLanguage: "تغییر زبان",
  persian: "فارسی",
  english: "انگلیسی",
  currentLanguage: "زبان فعلی",
  theme: "تم",
  lightTheme: "روشن",
  darkTheme: "تاریک",
  systemTheme: "سیستم",
  autoTheme: "خودکار",
  fontSize: "اندازه فونت",
  small: "کوچک",
  medium: "متوسط",
  large: "بزرگ",
  extraLarge: "خیلی بزرگ",
  resetToDefaults: "بازگشت به پیش‌فرض",
  saveSettings: "ذخیره تنظیمات",
  settingsSaved: "تنظیمات با موفقیت ذخیره شدند!",
  errorSavingSettings: "خطا در ذخیره تنظیمات",
  settingsDescription: "تنظیمات برنامه و حساب کاربری خود را مدیریت کنید",
  changePassword: "تغییر رمز عبور",
  currentPassword: "رمز عبور فعلی",
  deleteAccount: "حذف حساب کاربری",
  exportUserData: "خروجی داده‌های کاربر",
  importUserData: "ورود داده‌های کاربر",
  dataExported: "داده‌ها با موفقیت خروجی گرفته شدند",
  dataImported: "داده‌ها با موفقیت وارد شدند",
  confirmDeleteAccount: "آیا مطمئن هستید که می‌خواهید حساب کاربری خود را حذف کنید؟",
  accountDeleted: "حساب کاربری با موفقیت حذف شد",
  passwordChanged: "رمز عبور با موفقیت تغییر یافت",
  invalidCurrentPassword: "رمز عبور فعلی اشتباه است",
  // Notifications page translations
  notificationCenter: "مرکز اعلان‌ها",
  markAllAsRead: "همه را خوانده شده علامت بزن",
  clearAllNotifications: "پاک کردن همه اعلان‌ها",
  noNewNotifications: "اعلان جدیدی وجود ندارد",
  notificationPreferences: "ترجیحات اعلان‌ها",
  emailNotifications: "اعلان‌های ایمیل",
  pushNotifications: "اعلان‌های فوری",
  smsNotifications: "اعلان‌های پیامکی",
  mealReminders: "یادآوری وعده‌های غذایی",
  paymentNotifications: "اعلان‌های پرداخت",
  systemUpdates: "به‌روزرسانی‌های سیستم",
  promotionalOffers: "پیشنهادات تبلیغاتی",
  weeklyReports: "گزارش‌های هفتگی",
  enabled: "فعال",
  disabled: "غیرفعال",
  notificationTime: "زمان اعلان",
  beforeMealTime: "قبل از وعده غذایی",
  minutes: "دقیقه",
  hour: "ساعت",
  hours: "ساعت",
  notificationFrequency: "تکرار",
  immediately: "فوری",
  daily: "روزانه",
  weekly: "هفتگی",
  monthly: "ماهانه",
  never: "هرگز",
  testNotification: "ارسال اعلان آزمایشی",
  testNotificationSent: "اعلان آزمایشی ارسال شد",
  notificationSettingsSaved: "تنظیمات اعلان‌ها ذخیره شد",
  unreadNotifications: "خوانده نشده",
  readNotifications: "خوانده شده",
  allNotifications: "همه",
  notificationTitle: "اعلان",
  notificationMessage: "پیام",
  notificationDate: "تاریخ",
  notificationActions: "عملیات",
  markAsRead: "خوانده شده علامت بزن",
  deleteNotification: "حذف",
  notificationDeleted: "اعلان حذف شد",
  notificationsCleared: "همه اعلان‌ها پاک شدند",
  notificationTypes: "انواع اعلان",
  urgent: "فوری",
  info: "اطلاعات",
  warning: "هشدار",
  success: "موفقیت",
  error: "خطا",
};
